...
def issue(request):
    try:
        if request.session['adminid']!=None:
            ...
            status='pending'
            ib=IssueBook(bookid=bookid,isbnno=isbnno,program=program,branch=branch,year=year
                         ,bookname=bookname,authorname=authorname,name=name,rollno=rollno,
                         issuedate=issuedate,duedate=duedate,status=status,subject=subject)
            ib.save()
